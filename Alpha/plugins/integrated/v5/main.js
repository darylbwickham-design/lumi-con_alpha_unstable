...
module.exports = class LumiConBridgeIntegratedV5 extends Plugin {
  constructor(manifest, context) {
    super(manifest, context);

    this._server = null;

    // Dedupe for confirmed mode: deviceId -> lastSeq processed
    this._lastSeqByDevice = new Map();

    // Cached key labels, 0..35 (short/long + legacy fallback)
    ...
    // Cached device info (best-effort; filled if provided)
    this._deviceId = "";
    this._deviceIp = "";
    this._lastRssi = 0;
  }
  ...
  _formatStatusText(nowMs) {
    ...
    const ageSec = Math.max(0, Math.floor((nowMs - this._lastSeenMs) / 1000));
...
_refreshKeyLabels() {
  ...
  async _handleRequest(req, res) {
    try {
      ...
      // Update device status cache + variables
      const nowMs = Date.now();
      ...
      // One-line status string for overlays/debug (updates immediately on events)
