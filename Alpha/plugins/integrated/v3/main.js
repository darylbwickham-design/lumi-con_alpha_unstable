...
module.exports = class LumiConBridgeIntegratedV2 extends Plugin {
  constructor(manifest, context) {
    ...
    // Cached key labels (settings keyLabels), 0..35
    this._keyLabels = [];
...
_refreshKeyLabels() {
    this._keyLabels = parseKeyLabels(this.settings?.keyLabels);
  }

  _getKeyLabel(keyIndex) {
    const label = getKeyLabelFromList(this._keyLabels, keyIndex);
  ...
  async _handleRequest(req, res) {
    try {
      ...
      const keyLabel = this._getKeyLabel(keyIndex);
