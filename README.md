...
- **ESP8266**: Wi‑Fi + HTTP + optional TFT display + relays key events to **Lumia** via a plugin

This repo/docs cover **three supported builds**:

1) **6×6 ONLY (Buttons Only)** — Pico → ESP → Lumia plugin (alerts/variations)  
2) **CHAT RELAY ONLY (TFT Chat Bridge)** — Lumia plugin → ESP TFT (display messages)  
3) **INTEGRATED (Recommended / Pro)** — same ESP firmware + integrated plugin (events + TFT actions + device status)

Important note:

- **Build 1 and Build 3 use the same ESP8266 integrated firmware.**
- Build 1 simply does **not** use TFT actions (so **ESP Base URL** can be left blank).
...
- [Build 1: 6×6 ONLY (Buttons Only)](#build-1-66-only-buttons-only)
...
IMPORTANT (ESP flashing):

- When uploading to the ESP8266, disconnect **Pico TX (GP0) → ESP RX (GPIO3)** or the upload may fail.
...
- Lumia plugin triggers two alerts:
  - **6×6 short**
  - **6×6 long**
- Each alert supports **36 variations** (Key `0–35`)

### Firmware used (same as Integrated)

- Pico: `6x6_test_pico.ino`
- ESP: `lumi_con_esp_integrated_0_0_4.ino` (or your current integrated firmware)

### Plugin (Lumia)

- Install: **Lumi-Con Bridge (Integrated v5.2)** (`.lumiaplugin`)
- Minimal settings (buttons only):
  - Enable Listener = **ON**
  - Listen Port = **8787**
  - Shared Secret = **(blank)**
- You can leave **ESP Base URL** blank if you are not using a TFT.
...
Same ESP firmware + integrated Lumia plugin:

Device → PC (plugin listener)

- `POST http://<PC_IP>:8787/event` with JSON like `{ "event": 0..71 }`

Plugin → device (TFT actions)

- Display Add Line → `/msg` (Legacy GET) or `/ui` (UI POST)
- Display Set Status → `/status` (Legacy GET) or `/ui` (UI POST)
- Display Clear → `/clear` (Legacy GET) or `/ui` (UI POST)

Integrated plugins can also add:
...
Optional (if you set a secret):

- Shared Secret must match what the ESP sends.
...
The plugin exposes `key_label` and automatically picks the correct list based on short/long press.

---

## UI Mode: Legacy GET vs UI POST

This affects **TFT display only** (not key presses).
...
Wire TFT RST → ESP RST and set RST pin to `-1` in the constructor if needed.
...
- Plugin health (listener): `http://<PC_IP>:8787/health`
